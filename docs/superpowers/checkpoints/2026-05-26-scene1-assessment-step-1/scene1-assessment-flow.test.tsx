...
describe('scene1 assessment flow', () => {
  ...
  it('completes the KMI steps and shows the completion state', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '记不清了' }));
    await user.click(screen.getByRole('button', { name: '下一步' }));
    await user.click(screen.getByRole('button', { name: '中度' }));
    await user.click(screen.getByRole('button', { name: '情绪起伏明显' }));
    await user.click(screen.getByRole('button', { name: '下一步' }));
    ...
    await chooseInBlock('感觉异常', '轻度');
    await chooseInBlock('失眠', '中度');
    await chooseInBlock('易激动', '轻度');
    await chooseInBlock('忧郁', '无');
    await chooseInBlock('头晕', '轻度');
    await user.click(screen.getByRole('button', { name: '下一步' }));

    expect(screen.getByText('KMI 题组 2')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '完成评估' })).toBeInTheDocument();

    await chooseInBlock('易疲劳、乏力', '轻度');
    await chooseInBlock('关节或肌肉疼痛', '中度');
    await chooseInBlock('头痛频率', '轻度');
    await chooseInBlock('心悸', '无');
    await chooseInBlock('皮肤蚁走感', '轻度');
    await chooseInBlock('性生活影响', '轻度下降');
    await chooseInBlock('泌尿系统症状', '无');
    await user.click(screen.getByRole('button', { name: '完成评估' }));

    expect(screen.getByText('评估已完成')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '返回 scene1' })).toBeInTheDocument();
  });
